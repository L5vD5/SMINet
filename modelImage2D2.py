...
class qLayers(nn.Module):
    def __init__(self,branchLs,inputdim,n_layers=4):
        super(qLayers, self).__init__()
        ...
        for joint in range(n_joint):
            # Trackbool = branchLs[joint]
            setattr(self,'rev_pri_q_'+str(joint+1), q_layer(branchLs,inputdim))

    def forward(self,motor_control):
        n_joint = len(self.branchLs)
        batchSize = motor_control.size()[0]
        rev_q_list, pri_q_list = torch.Tensor([]).reshape(batchSize, -1), torch.Tensor([]).reshape(batchSize, -1)
        for joint in range(n_joint):
            q_layer = getattr(self,'rev_pri_q_'+str(joint+1))
            # print(q_layer(motor_control))
            rev_q, pri_q = q_layer(motor_control)
            # print(rev_q.size())
            rev_q_list = torch.cat([rev_q_list, rev_q], dim=1)
            pri_q_list = torch.cat([pri_q_list, pri_q], dim=1)

        # print(torch.cat([pri_q_list, rev_q_list], dim=1).shape)
        # return torch.cat([pri_q_list, rev_q_list], dim=1)
        return rev_q_list, pri_q_list
...
class Model(nn.Module):
    def __init__(self, branchNum, inputdim):
        ...
        self.q_layer = qLayers(branchLs, inputdim)
